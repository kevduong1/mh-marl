...
# bash ./training_scripts/gpu_servers/lbforage_runs/run_suite_iql.sh
seeds=(6342 1754 3388 4985 7301)
gpus=(0)
acting_policies=("average")
run_name="lbforaging_iql"
        ...
        for gpu_id in "${gpus[@]}"; do
          ...
          if [[ $process_count -lt 3 ]]; then
            ...
            wandb docker-run --name "$name" $gpu -ti -d --rm kevduong1/mh-marl:mh \
              ...
              target_update_interval_or_tau=200 use_rnn=True use_mh=$use_mh num_gammas=3 hyp_exp=0.1 acting_policy="$acting_policy" \

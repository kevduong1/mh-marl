...
# bash ./training_scripts/gpu_servers/lbforage_runs/run_suite_mappo.sh
seeds=(6342 1754 3388 4985 7301)
gpus=(2 3 4 5 6 7)
acting_policies=("single" "hyperbolic" "average" "largest")
run_name="lbforaging_mappo_mh"
        ...
        for gpu_id in "${gpus[@]}"; do
          ...
          if [[ $process_count -lt 3 ]]; then
            ...
            wandb docker-run --name "$name" $gpu -ti -d --rm kevduong1/mh-marl:mh \
              ...
              env_args.time_limit=25 t_max=20050000 env_args.key="$env_key" \

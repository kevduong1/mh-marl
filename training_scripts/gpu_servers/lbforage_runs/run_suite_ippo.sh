...
# bash ./training_scripts/gpu_servers/lbforage_runs/run_suite_ippo.sh
seeds=(6342 1754 3388 4985 7301)
gpus=(2 3)
acting_policies=("average")
run_name="lbforaging_ippo_mh_g-5"
        ...
        for gpu_id in "${gpus[@]}"; do
          ...
          if [[ $process_count -lt 4 ]]; then
            ...
            wandb docker-run --name "$name" $gpu -ti -d --rm kevduong1/mh-marl:mh \
              --config=ippo --env-config=gymma with \
              hidden_dim=128 lr=0.0003 entropy_coef=0.001 q_nstep=5 \
              target_update_interval_or_tau=200 use_rnn=False use_mh=$use_mh num_gammas=5 hyp_exp=0.1 acting_policy="$acting_policy" \
              standardise_rewards=False standardise_returns=False \
              env_args.time_limit=25 t_max=10050000 env_args.key="$env_key" \
